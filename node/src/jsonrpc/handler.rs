...
#[cfg(feature = "node")]
pub type MethodFnBox = Box<dyn Fn(Params, RpcMeta) -> Pin<Box<dyn Future<Output = Result<Value>> + Send>> + Send + Sync>;
...
pub fn methods() -> Vec<(Method, MethodFnBox)> {
    vec![
	(Method::ConnectPeerViaHttp, pin!(server::connect_peer_via_http)),
	(Method::ConnectPeerViaHttp, pin!(server::connect_peer_via_http)),
	...
	(Method::ClosePendingTransport, pin!(server::close_pending_transport)),
	(Method::SendTo, pin!(server::send_raw_message)),
	(Method::SendHttpRequestMessage, pin!(server::send_http_request_message)),
	(Method::SendSimpleText, pin!(server::send_simple_text_message)),
	(Method::SendCustomMessage, pin!(server::send_custom_message)),
	(Method::PublishMessageToTopic, pin!(server::publish_message_to_topic)),
	(Method::FetchMessagesOfTopic, pin!(server::fetch_messages_of_topic)),
