...
\begin{definition}[Chord performance baseline]
\[
\begin{array}{rcl}
\operatorname{fail}(Q,F)&=&\dfrac{|F|}{|Q|},\\[8pt]
\operatorname{timeout}(Q)&=&\dfrac{1}{|Q|}\sum_{q\in Q}to(q),\\[8pt]
\eta&=&(|N|,\mathsf{churn},\mathsf{stabilize},\mathsf{loss}),\\[4pt]
B_C(\eta)&=&
  (\mathbb{E}[h_C],
   \operatorname{timeout}(Q_C),
   \operatorname{fail}(Q_C,F_C)),\\[4pt]
\mathbb{E}[h_C]&\in&O(\log |N|).
\end{array}
\]
\end{definition}
...
\begin{aligned}
  ...
  (&N,\rho,r,\mathsf{transport},\mathsf{runtime},\\
   &\mathsf{scheduler},\vec{T}_R,
     \operatorname{timeout}(Q_R),
     \operatorname{fail}(Q_R,F_R)).
\end{aligned}
\]
\[
  B_C(\eta)
...
\[
\begin{array}{rcl}
\vec{T}_R&=&(T_{conn},h_R T_{hop},T_{crypto},\\
&&T_{app},T_{witness}),\\
...
\mathsf{PerfClaim}_{R}
  &\Rightarrow&
  \mathsf{Measure}_{build}(\mathsf{Report}_{Rings})\\
&&{}\wedge\mathsf{Cite}(B_C).
\end{array}
\]
Chord supplies the lookup/churn baseline~\cite{Chord}; Rings performance
requires Rings-specific measurements.
