...
\date{Created: February 8, 2023\\Last updated: July 4, 2026}
...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
      ...
      Rings Network is a decentralized peer-to-peer network for the sovereign age. It describes a target architecture in which browsers and native devices participate as first-class nodes, identities are addressed as DIDs on a Chord overlay, resources and services are mapped into the same identifier space, and applications compose privacy-preserving protocols such as encrypted messaging, offline mailboxes, hidden service descriptors, ranking, and sidecar ordering. This paper presents the ideal Rings model while updating directions that have been superseded: Chord identifiers are treated as an additive circular space for routing and placement, cryptographic groups and fields are separate protocol carriers, and message relay, ranking, and sequencing are specified as Rings protocols rather than inherited wholesale from unrelated systems.
...
\subsection{Identity and Runtime Surfaces}
...
In the protocol layer, the central design concept revolves around the utilization of a Decentralized Identifier (DID). A DID is a $2^{160}$-point Chord identifier represented as an element of the additive cyclic group $Z / 2^{160}$. Addition, subtraction, and negation model clockwise offsets, biased ordering, finger targets, and affine replica placement. DID multiplication is outside the Rings routing model: cryptographic multiplication belongs to the fields or groups used by specific cryptographic protocols, not to Chord identifiers themselves.

However, elements in a circular identifier space cannot be compared by a single global less-than relation that is meaningful for every observer. The BiasDID algorithm addresses this limitation. The algorithm selects a fixed point C and compares A and B by their clockwise offsets from C. In this manner, ordering is always relative to the observer or operation that chose the fixed point.
...
In Rings Network, the representation of Decentralized Identifiers (DIDs) is achieved through a fixed-width Chord identifier space. A DID is represented by a 160-bit value, and Chord interval comparisons are relative to an observer. Rings uses biased ordering to compare positions from a chosen reference point, and additive offsets for finger targets and redundant placement. This is additive cyclic-group arithmetic over $Z / 2^{160}$, not protocol-level finite-field multiplication.
...
\]
The successor sequence of $n$ is the bounded list of the nearest nodes by increasing $\operatorname{dist}_n$, and the predecessor is the farthest forward node, which is equivalent to the nearest node behind $n$ on the circle. A finger slot $i$ is the nearest known node whose clockwise distance is at least $2^i$; Rings uses a sparse no-wrap finger table, so a high slot may be empty when the known topology does not contain such a node.
...
Rings also models side effects algebraically. A DHT transition should not directly perform network IO. It returns an action value such as find-successor, query-successor-list, try-connect, notify, sync-storage, or no-op. The message layer treats these values as a small action functor and lowers them into the coproduct of core effects: payload relay, fresh message send, and connection management. The interpreter then executes those effects against the current transport implementation.

This is a monadic boundary in the operational sense: pure protocol logic builds an effect description, composition sequences descriptions such as multi-actions, and an interpreter finally binds the description to IO. The target architecture does not require every runtime to expose a generic Monad trait, but it does require the same separation of pure state transition, effect value, and interpreter. This keeps protocol laws testable without opening real sockets, while allowing browser, native, and embedded runtimes to use different effect interpreters.

\subsection{Rings Extension: Formal Verification Boundary}

The Rings formal-methods style starts from the Chord and Correct Chord literature, then writes the Rings state machine in TLA-like state variables and operators. The state variables are the DID carrier $R = Z / 2^{160}$, the successor sequence, predecessor, sparse finger table, and replicated entry state. The operators are join, rectify, stabilize, finger repair, storage sync, storage repair, and lookup. The main invariant is that the converged local state is the unique fixpoint determined by the sorted circular topology.
...
\label{alg:join}
\begin{algorithmic}[1]
\Function{JoinThenSync}{$n, known$}
\State $s\gets known.find\_successor(n.did)$
\State $t\gets query\_topology(s)$
\State $succ[n]\gets take(K, s :: t.succ)$
\State $pred[n]\gets none$
\State \textbf{emit} $QuerySuccList(s)$
\If{$head(succ[n]) \neq none$}
...
When a participant joins the network, it uses an existing peer as an entry point, asks the overlay to locate its successor, initializes a bounded successor sequence, and then emits follow-up actions for synchronization and notification. The join transition does not directly mutate remote peers. It describes the local state update and the messages that the effect interpreter must deliver.
...
\Function{Lookup}{$key, node$}
    ...
    \If{$next = none$}
        \State \Return $Local(s)$
    \Else
        \State \Return $Remote(next)$
        \State \textbf{effect} $FindSuccessor(key)$
    \EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
...
The \textbf{lookup} algorithm locates the successor of a DID or VID. If the local successor sequence can answer the interval query, the transition returns a local owner. Otherwise it returns a remote action naming the next hop and the requested key. On a converged ring, repeatedly interpreting the remote action resolves to the mathematical successor of the target. The $O(log N)$ routing expectation depends on sufficiently populated finger tables; correctness still falls back to the successor sequence.
...
\EndIf
\State $succ[n]\gets take(K, merge(succ[n], topo.succ))$
\If{$head(succ[n]) \neq none$}
\State \textbf{emit} $Notify(head(succ[n]), n.did)$
\EndIf
\EndProcedure
...
Given an elliptic-curve cyclic group $\mathbb{G}$ of order $q$ with generator $g$, a private key $x \in \mathbb{Z}_q$, and public key $h = g^x$, the ElGamal operation can be summarized as follows:
