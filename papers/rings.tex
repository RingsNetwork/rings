...
\author{Ryan~J.~Kung%
\thanks{Ryan J. Kung can be reached at ryankung@ieee.org. Manuscript created February 8, 2023; last updated July 5, 2026.}}
...
\normalfont
Rings is an implemented structured peer-to-peer network specified by carriers,
owner maps, protocol morphisms, and explicit obligations.  Its motivating
failure mode is authority collapse:
\[
  \mathsf{root}(id,data,route)
  \in\{\mathsf{server},\mathsf{dns},\mathsf{relay},\mathsf{platform}\}.
\]
Rings replaces that root with self-certifying identity and resource ownership
over a CorrectChord overlay, while keeping cryptographic carriers disjoint from
the routing carrier.  The object studied here is
\[
  \RingsObj=
...
\]
\[
  \{D,V,M,H,Q,W\}\subset\operatorname{Obj}(\SetOverR),
  \qquad
...
operators~\cite{Kshetri2013,Zissis2012,KshetriMurugesan2013}.  Rings starts
from the opposite requirement:
\[
\begin{array}{rcl}
\mathsf{Own}(d)&=&k_d,\\
\mathsf{Route}(x)&=&\operatorname{owner}_N(H_\nu(x)),\\
...
The central modeling choice is carrier separation.  The Chord identifier space
is the additive cyclic group for placement and interval order; signatures,
threshold schemes, encryption, and zero-knowledge protocols remain in their own
fields or groups:
\[
\begin{array}{rcl}
R&=&\mathbb{Z}/2^{160}\mathbb{Z},\\
...
\[
\begin{array}{rcl}
\mathsf{Kademlia}&\Rightarrow&\mathsf{lookup}_{XOR}
  \not\Rightarrow\mathsf{clockwiseOwner},\\
\mathsf{RelayMixnet}&\Rightarrow&\mathsf{pathPrivacy}
  \not\Rightarrow\mathsf{structuredOwner},\\
\mathsf{Ledger}&\Rightarrow&\mathsf{globalOrder}
  \not\Rightarrow\mathsf{localOverlay}.
\end{array}
\]
...
\RingsObj&\models&
G_{id}\land G_{own}\land G_{io}
\land G_{eff}\land G_{cr}\land G_{rep}.
\end{array}
\]
\end{obligation}
...
\end{array}
\]
The prose in this paper is used only where the construction needs motivation,
scope, or comparison; the claims themselves are carried by definitions,
constraints, invariants, algorithms, and obligations.
\end{definition}

\[
\begin{aligned}
\mathsf{Contribution}=\{&
R,\SetOverR,\Omega_N,\mathcal{R}_N,
\mathsf{RepKey}^{\rho},\\
...
|P_k|&=&\rho,\\
|O_k|&\leq&\min(\rho,|N|),\\
\mathsf{collapse}_{N,\rho}(k)&\Leftrightarrow&|O_k|<\rho.
\end{array}
\]
Thus replica keys specify redundancy intent; live owner images specify realized
redundancy.  CorrectChord successor lists remain the topology safety root.
\end{definition}
...
\[
\begin{array}{rcl}
K_{\nu,x}&=&\operatorname{replicaKeys}(H_\nu(x)),\\[2pt]
...
\mathsf{Collapse}_{\nu,x}&\Leftrightarrow&L_{\nu,x}<\min(\rho,|N|),\\
\mathsf{LiveClaim}_{\nu,x}(\rho)&\Rightarrow&\mathsf{FullRed}_{\nu,x},\\
\mathsf{Report}_{Rings}&\ni&
...
\operatorname{SeqOK}_{\Delta}(seq,last)&\Leftrightarrow&
  \mathsf{Dup}_{\Delta}(seq)\\
&&{}\lor\mathsf{New}_{\Delta}(seq,last),\\
\mathsf{Dup}_{\Delta}(seq)&\Rightarrow&\mathsf{emit}=\epsilon.
\end{array}
\]
...
\begin{tabular}{p{1.05cm}|p{2.0cm}|p{2.45cm}}
\hline
Attack & Predicate & Required boundary \\ \hline
Sybil & many DIDs, one actor & admission and ranking policy \\
Replay & stale valid $\sigma$ & TTL, nonce cache, stream/final \\
MITM & wrong key for DID & DID proof and signed envelope \\
DoS & unbounded resource use & quotas and chunk bounds \\
\hline
\end{tabular}
\caption{Security predicates and required controls}
...
\begin{array}{rcl}
\mathsf{Base}&\not\vdash&
  \mathsf{SybilResist}\land\mathsf{Anon}\land\mathsf{DoSResist},\\
\mathsf{SecClaim}&\Rightarrow&
  \mathsf{Adm}\land\mathsf{Rank}
...
\[
  \mathsf{Spec}\neq\mathsf{Evidence}_{\Gamma}\neq\mathsf{Evaluation}.
\]
The repository contains executable evidence for finite overlay, storage, and
...
\]
\[
  \mathsf{Explore}_{\Gamma}=\mathsf{ModelCheck}(\to,\Gamma)
\]
\[
\begin{array}{rcl}
...
\hline
Artifact & Checked scope & Limitation \\ \hline
Topology tests
  & join, rectify, stabilize fixpoints on finite DID layouts
  & not a parametric TLAPS proof \\
Stateright
  & predecessor update, small discovery abstraction, storage CRDT, handoff
  & scoped models, not a full live network \\
Trace replay
...
\hline
\end{tabular}
\caption{Verification evidence and explicit limits}
...
\[
\begin{array}{rcl}
F_X(\eta;D)&\subseteq&Q_X(\eta;D),\\
to_X&:&Q_X(\eta;D)\to\mathbb{N},\\
\phi_X(\eta;D)&=&|F_X(\eta;D)|/|Q_X(\eta;D)|,\\[3pt]
\bar{to}_X(\eta;D)&=&|Q_X|^{-1}\sum_{q\in Q_X}to_X(q),\\[3pt]
B_X(\eta;D)&=&
  (\mathbb{E}[h_X^f\mid\eta,D],
   \mathbb{E}[h_X^r\mid\eta,D],\\
&& \bar{to}_X(\eta;D),\phi_X(\eta;D)),\\
h_X^r&=&h_X^f+1.
\end{array}
\]
\[
\begin{array}{rcl}
D_{656}&=&D_C^{paper}\cup D_C^{1600}\cup D_R^{docker},\\
D_C^{paper}&=&\{C_{II},C_{III},C_{IV},C_{8},C_{9},C_{10}\},\\
D_C^{1600}&=&\{\mathsf{maintained},\mathsf{stale}\},\\
D_R^{docker}&=&\{\mathsf{convergence},\mathsf{transport}\}.
\end{array}
\]
\[
\begin{array}{rcl}
C_{II}&:&(N,r,|Q|)=(1000,20,10^4),\\
C_{III}&:&(N,r,\lambda) \simeq (1000,20,0.05..0.40),\\
D_C^{1600}&:&(N,r,m,L)=(1600,3,160,64).
\end{array}
\]
\[
  D_C^{paper}\not\subset\mathsf{Impl}_{Rings},
  \qquad
  D_R^{docker}\subset\mathsf{Impl}_{Rings}.
\]
Here $X\in\{C,R\}$, $\eta$ fixes the workload and fault regime, and $D$ fixes
the artifact.  $h^f$ counts forwarding hops; $h^r$ counts lookup rounds.
\end{definition}

Table~\ref{chord-paper-data-table} gives the PR~656 Chord-paper coordinates
($N=1000,r=20$); it is not Rings runtime evidence.
...
\hline
\end{tabular}
\caption{Chord-paper-aligned rows in
\texttt{chord-paper-sim-2026-07-05.jsonl}.}
\label{chord-paper-data-table}
\end{table}

Table~\ref{rings-param-chord-data-table} fixes Rings' current Chord parameters
($N=1600,r=3,m=160$).  Maintained rows are the baseline; stale rows are
no-maintenance sensitivity.
...
\hline
$\eta$ & $M$ & $\bar h^f/\bar h^r$ &
$ok$ & $\bar{to}$ & $10^4\phi$ \\ \hline
stable & m & $4.469/5.469$ & 1.000 & 0.000 & 0.0 \\
$f=.10$ & m & $4.394/5.394$ & 1.000 & 0.000 & 0.0 \\
$f=.20$ & m & $4.316/5.316$ & 1.000 & 0.000 & 0.0 \\
$\chi=.05$ & m & $4.465/5.465$ & 1.000 & 0.000 & 0.0 \\
$\chi=.40$ & m & $4.470/5.470$ & 1.000 & 0.000 & 0.0 \\
$f=.10$ & s & $4.758/5.758$ & .990 & .734 & 98.2 \\
$f=.20$ & s & $5.186/6.186$ & .969 & 1.890 & 314.7 \\
$\chi=.05$ & s & $4.617/5.617$ & .945 & .361 & 546.2 \\
$\chi=.40$ & s & $5.746/6.746$ & .484 & 4.604 & 5161.1 \\
\hline
\end{tabular}
\caption{Current Rings-parameter Chord baselines
($m=$maintained, $s=$stale; churn endpoints) from
\texttt{dummy-paper-scale-2026-07-05.jsonl}.}
...
\begin{aligned}
  \mathsf{Report}_{Rings} =
  (&N,\rho,r,\mathsf{transport},\mathsf{runtime},\\
   &\mathsf{scheduler},\vec{T}_{total},
     B_R(\eta_R;D_R^{docker})).
\end{aligned}
\]
\[
  B_C(\eta;D_C^{paper})
  ...
  B_R(\eta_R;D_R^{docker}).
\]
The report keeps two ledgers: $B_C$ is the paper baseline; $B_R$ is Rings
runtime evidence.
\end{obligation}
...
&&T_{app},T_{witness}),\\
\sum_i\vec{T}_{total,i}&=&T_{total},\\
\mathsf{Overhead}_{R/C}
  &=&(T_{conn},T_{crypto},T_{app},\\
&&T_{witness},\mathsf{runtime}),\\
\mathsf{PerfClaim}_{R}
  &\Rightarrow&
  \mathsf{Measure}_{build}(B_R)\\
&&{}\wedge\mathsf{Cite}(B_C^p),\\
D_C^p&=&D_C^{paper},\\
B_C^p&=&B_C(\eta;D_C^p).
\end{array}
\]
...
\begin{array}{rcl}
D_R^{conv}&=&
  (16,309.532s,0/16,3/16,\\
&& .194,1.026,8058.6),\\
D_R^{tx}&=&
\{(1,10.46),(16,107.35),\\
&& (64,90.36)\}_{\mathrm{KiB},\mathrm{Mbps}}.
\end{array}
\]
Chord supplies the lookup/churn baseline~\cite{Chord}; Rings supplies runtime
evidence only through Rings measurements.  The Docker/WebRTC cluster is
convergence evidence, not steady-state lookup; $D_R^{tx}$ is payload throughput.
...
\omega&=&\mathsf{owner},\\
\chi&=&\mathsf{order},\\
\lambda&=&\mathsf{runtime}.
\end{array}
\]
\[
\begin{array}{c|c}
S&\mathsf{Axis}(S)\\ \hline
\mathsf{Kad}&(XOR,\mathsf{bucket},\bot,\mathsf{native})\\
\mathsf{Chord}&(\mathsf{circ},\mathsf{succ},\bot,\mathsf{native})\\
\mathsf{Tor/Nym}&(\mathsf{path},\bot,\bot,\mathsf{relay})\\
\mathsf{Ledger}&(\bot,\mathsf{acct},\mathsf{global},\mathsf{repl})\\
\mathsf{Rings}&(\mathsf{circ},\operatorname{owner}_N,\mathsf{sidecar},
  \mathsf{web}\cup\mathsf{native})
\end{array}
\]
Kademlia/IPFS/libp2p supply scalable lookup but not the clockwise owner law
needed here~\cite{Kademlia,IPFS,Libp2p}.  Tor and Nym supply relay-path
privacy, not structured ownership~\cite{TorNetwork,NymNetwork}.  Ledgers
supply global order; Rings keeps ordering outside the base overlay.  WebRTC and
WebAssembly are runtime adapters, so the same protocol object is interpreted in
browser and native environments~\cite{webrtc-standard,webassembly}.

\begin{table}[htbp]
\centering
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{p{1.25cm}|p{1.25cm}|p{2.95cm}}
\hline
...
Rings
  & circular & clockwise owner, successor list, affine placement keys \\
\hline
\end{tabular}
\vspace{2pt}
...
The paper's claim is the following structured object, not an informal bundle of
network mechanisms:
...
\mathsf{Evidence}&=&
  \{\mathsf{scopedModels},\mathsf{traceReplay}\}\\
&&\cup\{\mathsf{invariants},\mathsf{benchmarkObligations}\}.
\end{array}
\]
