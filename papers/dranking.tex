...
% A single text column keeps formal definitions and long equations readable.
...
\usepackage{bookmark}
\hypersetup{
  pdftitle={The DRanking Protocol: Verifiable Distributed Ranking and Sybil-Bounded Admission for Structured Peer-to-Peer Networks},
  pdfauthor={Ryan J. Kung},
  pdfsubject={Distributed ranking and admission protocol design}
}
% Ragged-right table cells avoid stretched spacing in technical descriptions.
...
\title{The DRanking Protocol:\\[0.3em]
\large Verifiable Distributed Ranking and Sybil-Bounded Admission\\
...
\noindent\textbf{Keywords:} peer-to-peer networks; service receipts; trust propagation;
Sybil resistance; verifiable computation.

\medskip
\noindent\textbf{Implementation scope.} This paper specifies the proposed protocol.
The Rings implementation currently collects provisional liveness-probe receipts;
it does not implement the complete finalized ledger, trust algebra, committee
selection, or admission policy described here. See the
\href{https://github.com/RingsNetwork/rings/blob/master/docs/src/advanced-topic/dranking-service-receipts.md}{repository's service-receipt documentation}.
...
\draw[->] (b) -- node[above,font=\scriptsize]{$\S\ref{sec:sampling}$} (sor);
\draw[->] (sor) -- (com);
\draw[->] (com) -- (qc);
\draw[->] (t) -- (sor);
\end{tikzpicture}
...
The deployed vertical slice is deliberately narrower than
Definition~\ref{def:receipt}: it accepts only a versioned one-unit probe,
countersigned by distinct provider and beneficiary account roles.  Its claim
binds the overlay, provisional wall-clock epoch, fresh nonce, and digests of
the exact signed request and completion transactions.  Live collection and
later cryptographic verification are separate predicates, and retained
evidence is bounded local input only: it does not update credit, routing, the
receipt graph, or trust.  A finalized input-block receipt will use a distinct
canonical format and signing domain rather than reinterpret the provisional
wire.
