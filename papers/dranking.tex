...
\[
\begin{array}{rcl}
\mathsf{TSig}_S&:&\theta\text{-of-}|S|,\ \text{\emph{unique}~\cite{BLS}},\\
|\{\sigma:\mathsf{TVrfy}(k,\sigma)=\top\}|&=&1\quad\forall k,\\
B_k&=&\mathsf{h}\bigl(\mathsf{TSig}_S(k)\bigr).
\end{array}
\]
\end{definition}
...
\label{alg:sortition}
\begin{algorithmic}[1]
\State $B_k\gets\mathsf{h}(\mathsf{TSig}_S(k))$
