...
pub trait TransportManager<E: Send, EvCh: Channel<E>> {
    ...
    fn remove_transport(&self, address: &Address) -> Option<(Address, Arc<Self::Transport>)>;
    fn get_transport_numbers(&self) -> usize;
    fn get_addresses(&self) -> Vec<Address>;
    fn get_transports(&self) -> Vec<(Address, Arc<Self::Transport>)>;
...
#[cfg_attr(feature = "wasm", async_trait(?Send))]
#[cfg_attr(not(feature = "wasm"), async_trait)]
pub trait MessageHub<'a, E: Send, M: Send, EvCh: Channel<E>, MsgCh: Channel<SendMessage<M>>> {
    type Transport: IceTransport<E, EvCh>;

    fn new(
        transport_manager: &'a dyn TransportManager<E, EvCh, Transport = Self::Transport>,
