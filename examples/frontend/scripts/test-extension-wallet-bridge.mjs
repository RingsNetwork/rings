...
try {
    ...
    await extensionPage.evaluate(() => globalThis.RingsExtensionNodeBridge.stop().catch(() => null));
    ...
    await fixturePage.waitForTimeout(250);
    await chooseEip191Wallet(fixturePage, "metamask");
    const eip191ConnectPromise = extensionPage.evaluate(() => globalThis.RingsExtensionWalletBridge.connect("eip191"));
    assert.equal(await fixturePage.locator("#rings-eip191-provider-chooser").count(), 0);
    const eip191Connect = await eip191ConnectPromise;
    const eip191Sign = await extensionPage.evaluate(() => globalThis.RingsExtensionWalletBridge.sign("eip191", "rings test proof", "0x1234567890abcdef1234567890abcdef12345678"));
    const ed25519Connect = await extensionPage.evaluate(() => globalThis.RingsExtensionWalletBridge.connect("ed25519"));
    const ed25519Sign = await extensionPage.evaluate(() => globalThis.RingsExtensionWalletBridge.sign("ed25519", "rings test proof"));
    await fixturePage.bringToFront();
    await fixturePage.waitForTimeout(250);
    await chooseEip191Wallet(fixturePage, "metamask");
    const legacyEip191ConnectPromise = extensionPage.evaluate(() => globalThis.RingsExtensionWalletBridge.connect("metamask"));
