...
const jsFile = entryFileFromHtml(sourceHtml, files, /import\s+init[\s\S]*?from\s+['"]([^'"]+\.js)['"]/) ??
...
const wasmFile = entryFileFromHtml(sourceHtml, files, /module_or_path:\s*['"]([^'"]+_bg\.wasm)['"]/) ??
...
await writeFile(join(extensionDist, "offscreen.html"), htmlShell(jsFile, wasmFile, { includeNodeBridge: false }), "utf8");
...
await writeFile(join(extensionDist, "manifest.json"), `${JSON.stringify(manifest(extensionVersion), null, 2)}\n`, "utf8");
console.log(`Packaged Chrome MV3 extension at ${extensionDist}`);
/**
 * Finds exactly one generated file matching a predicate.
 */
function singleFile(fileList, predicate, label) {
 ...
 */
function entryFileFromHtml(html, fileList, pattern) {
...
async function renderSvgToPng(svgPath, pngPath, size) {
    ...
    throw new Error(`Unable to render ${svgPath} to ${pngPath}. Tried sips, rsvg-convert, and magick.\n${errors.join("\n")}`);
}
/**
 * Builds the extension page HTML without inline scripts so MV3 CSP accepts it.
 */
function htmlShell(jsFileName, wasmFileName, options) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
 ...
 */
function bootstrapScript(jsFileName, wasmFileName) {
