...
#[cfg(all(feature = "dummy", not(target_family = "wasm")))]
#[tokio::test]
async fn disconnected_open_transport_cannot_commit_admission() -> Result<()> {
    let transport = Arc::new(transport_with_measure(Arc::new(
        RecordingMeasure::default(),
    ))?);
    let peer = SecretKey::random().address().into();
...
#[cfg(all(feature = "dummy", not(target_family = "wasm")))]
#[tokio::test]
async fn non_ready_transport_is_admitted_but_not_routable_or_live() -> Result<()> {
...
#[cfg(all(feature = "dummy", not(target_family = "wasm")))]
#[tokio::test]
async fn tracked_send_rejects_disconnected_open_transport() -> Result<()> {
    let (transport, peer) = admitted_dummy_transport().await?;
    transport
        .force_peer_connection_state_without_callback(peer, WebrtcConnectionState::Disconnected)?;
    transport.force_peer_data_channel_open_without_callback(peer, Some(true))?;
