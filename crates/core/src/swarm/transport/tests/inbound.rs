...
#[tokio::test]
async fn transport_preparation_authenticates_every_reserved_lane() -> Result<()> {
    let transport = Arc::new(transport_with_measure(Arc::new(
        RecordingMeasure::default(),
    ))?);
    let peer_key = SecretKey::random();
...
#[tokio::test]
async fn reassembled_control_shape_is_verified_before_lane_transition() -> Result<()> {
    let transport = Arc::new(transport_with_measure(Arc::new(
        RecordingMeasure::default(),
    ))?);
...
#[tokio::test]
async fn malformed_transport_preparation_records_peer_receive_failure() -> Result<()> {
...
#[tokio::test]
async fn invalid_chunk_shape_records_peer_receive_failure() -> Result<()> {
    let measure = Arc::new(RecordingMeasure::default());
    let transport = Arc::new(transport_with_measure(measure.clone())?);
    let peer_key = SecretKey::random();
    let peer: Did = peer_key.address().into();
...
#[tokio::test]
async fn reassembly_rejections_score_only_invalid_input_and_release_resources() -> Result<()> {
    let measure = Arc::new(RecordingMeasure::default());
...
#[tokio::test]
async fn expired_partial_reassembly_releases_shared_budget_without_more_peer_traffic() -> Result<()>
{
    let limits = ReassemblyLimits {
        max_pending_messages: 1,
        max_chunk_data_len: 1_024,
    ...
    let frame = local_wire(
        Message::Chunk(Chunk {
            ...
            meta: crate::chunk::ChunkMeta {
                ttl_ms: 20,
                ..crate::chunk::ChunkMeta::default()
            },
        }),
        &session,
        transport.dht.did,
...
#[tokio::test]
async fn invalid_transport_envelope_records_peer_receive_failure() -> Result<()> {
    let measure = Arc::new(RecordingMeasure::default());
    let transport = Arc::new(transport_with_measure(measure.clone())?);
    let peer: Did = SecretKey::random().address().into();
    let callback = InnerSwarmCallback::new(transport, Arc::new(NoopSwarmCallback));
