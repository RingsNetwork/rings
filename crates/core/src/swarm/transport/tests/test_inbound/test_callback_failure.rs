...
#[tokio::test]
async fn test_validation_deadline_drops_user_future_releases_capacity_and_unblocks_lane(
) -> Result<()> {
...
#[tokio::test]
async fn test_inbound_callback_deadline_drops_user_future_and_releases_capacity() -> Result<()> {
    let transport = Arc::new(transport_with_measure(Arc::new(
        RecordingMeasure::default(),
    ))?);
    let key = SecretKey::random();
...
#[tokio::test]
async fn test_actor_panic_drops_active_and_queued_capacity_and_closes_mailbox() -> Result<()> {
