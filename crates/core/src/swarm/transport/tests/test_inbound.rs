...
#[tokio::test]
async fn test_origin_quota_is_shared_across_relays_but_isolated_between_origins() -> Result<()> {
    ...
    assert!(matches!(
        ...
        Some(Error::OriginQuotaMessageRateExhausted { .. })
    ));
...
#[tokio::test]
async fn test_normal_and_reassembled_messages_each_consume_one_logical_byte_cost() -> Result<()> {
    let local = SessionSk::new_with_seckey(&SecretKey::random())?;
    ...
    assert!(matches!(
        error.downcast_ref::<Error>(),
        Some(Error::OriginQuotaByteRateExhausted { .. })
    ));
