...
#[test]
fn event_disposition_is_defined_by_the_active_generation_only() -> Result<()> {
...
#[test]
fn finger_candidate_admission_is_total_over_lifecycle_and_routability() {
    let peer = SecretKey::random().address().into();
...
#[test]
fn pending_admission_model_and_registry_reject_generation_exhaustion_without_reuse() {
    let peer = SecretKey::random().address().into();
