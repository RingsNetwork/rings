...
const _: () = {
    assert!(
        TRANSPORT_TIMEOUT_PROFILE.send_accept.as_millis()
            <= TRANSPORT_TIMEOUT_PROFILE.delivery.as_millis()
    );
    assert!(
        TRANSPORT_TIMEOUT_PROFILE.send_accept.as_millis()
            <= TRANSPORT_TIMEOUT_PROFILE.tracked_payload.as_millis()
    );
    assert!(
        TRANSPORT_TIMEOUT_PROFILE.close.as_millis()
            <= TRANSPORT_TIMEOUT_PROFILE.delivery.as_millis()
    );
    assert!(
        TRANSPORT_TIMEOUT_PROFILE.close.as_millis()
