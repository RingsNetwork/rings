...
#[test]
fn test_wake_before_first_poll_is_retained() {
    let queue = queue(1);
...
#[test]
fn test_cancelling_armed_front_hands_wake_to_successor() {
    let queue = queue(2);
...
#[test]
fn test_cancelling_middle_preserves_fifo_release_order() {
    let queue = queue(3);
