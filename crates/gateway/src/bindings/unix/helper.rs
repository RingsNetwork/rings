...
/// Serve one connected gateway lifecycle, then remove the control socket and return.
///
/// The helper remains a foreground process. It does not install or invoke a service manager.
pub fn serve(mut options: UnixHelperOptions) -> Result<(), GatewayError> {
    ...
    let listener = UnixListener::bind(&options.socket_path)
        .map_err(|error| platform_error("bind-unix-helper", error))?;
...
fn serve_listener(
    ...
    let mut stream = accept_authorized(&listener, authorized_uid)?;
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|error| platform_error("build-unix-helper-runtime", error))?;
    let mut control = NativeTunnelControl::new(tunnel_options)?;
    let mut active: Option<(String, NativeTunnelLease)> = None;

    loop {
        let request = match read_request(&mut stream) {
            Ok(Some(request)) => request,
            Ok(None) => return cleanup_disconnected(&runtime, &mut control, active),
            Err(error) => {
                let cleanup = cleanup_disconnected(&runtime, &mut control, active);
                return combine_results(Err(error), cleanup, "read-unix-helper-request");
            }
        };
        match request {
            UnixConfigRequest::Establish {
                ...
                if active.is_some() {
                    send_failure(
                        &mut stream,
                        "establish",
                        "a helper tunnel lease is already active".to_string(),
                    )?;
                    continue;
                }
                if let Err(error) =
                    runtime.block_on(control.replace_bypass_targets(&underlay_targets))
                {
                    send_failure(&mut stream, "replace-underlay-bypass", error.to_string())?;
                    continue;
                }
                let established = match runtime.block_on(control.establish(&plan)) {
                    Ok(established) => established,
                    Err(error) => {
                        send_failure(&mut stream, "establish", error.to_string())?;
                        continue;
                    }
                };
                ...
                let lease_id = format!("{}-1", std::process::id());
                let response = UnixConfigResponse::Established {
                    lease_id: lease_id.clone(),
                    interface_name,
                };
                if let Err(error) =
                    send_response(&mut stream, &response, Some(descriptor.as_raw_fd()))
                {
                    let cleanup = runtime.block_on(control.teardown(lease));
                    return combine_results(Err(error), cleanup, "send-unix-helper-establish");
                }
                active = Some((lease_id, lease));
            }
            UnixConfigRequest::ReplaceBypass {
                lease_id,
                underlay_targets,
            } => {
                if active.as_ref().map(|(id, _)| id.as_str()) != Some(lease_id.as_str()) {
                    send_failure(
                        &mut stream,
                ...
                match runtime.block_on(control.replace_bypass_targets(&underlay_targets)) {
                    Ok(()) => send_response(&mut stream, &UnixConfigResponse::Updated, None)?,
                    Err(error) => {
                        send_failure(&mut stream, "replace-underlay-bypass", error.to_string())?
                    }
                }
            ...
            UnixConfigRequest::Teardown { lease_id } => {
                ...
                let Some((_, lease)) = active.take() else {
                    send_failure(
                        &mut stream,
                        "teardown",
                        "no helper tunnel lease is active".to_string(),
                    )?;
                    continue;
                };
                match runtime.block_on(control.teardown(lease)) {
                    Ok(()) => {
                        send_response(&mut stream, &UnixConfigResponse::TornDown, None)?;
                        return Ok(());
                    }
                    Err(error) => {
                        send_failure(&mut stream, "teardown", error.to_string())?;
                        return Err(error);
                    }
                }
            }
...
fn accept_authorized(
    ...
    loop {
        let (stream, _) = listener
            .accept()
            .map_err(|error| platform_error("accept-unix-helper", error))?;
...
fn peer_uid(stream: &UnixStream) -> Result<u32, GatewayError> {
    nix::sys::socket::getsockopt(stream, nix::sys::socket::sockopt::PeerCredentials)
        ...
        .map_err(|error| platform_error("authenticate-unix-helper-peer", error))
}
...
fn cleanup_disconnected(
    ...
    active: Option<(String, NativeTunnelLease)>,
) -> Result<(), GatewayError> {
    match active {
        Some((_, lease)) => runtime.block_on(control.teardown(lease)),
        None => Ok(()),
    }
}
...
fn configure_socket(options: &UnixHelperOptions) -> Result<(), GatewayError> {
    std::fs::set_permissions(&options.socket_path, std::fs::Permissions::from_mode(0o600))
        .map_err(|error| platform_error("chmod-unix-helper", error))?;
    if let Some(uid) = options.socket_owner {
        chown(&options.socket_path, Some(Uid::from_raw(uid)), None)
            .map_err(|error| platform_error("chown-unix-helper", error))?;
    }
    Ok(())
}
...
fn secure_child_path(
    ...
    let canonical_parent =
        std::fs::canonicalize(parent).map_err(|error| platform_error(operation, error))?;
...
fn remove_stale_socket(path: &Path) -> Result<(), GatewayError> {
    let metadata = match std::fs::symlink_metadata(path) {
        ...
        Err(error) => return Err(platform_error("inspect-unix-helper-socket", error)),
    };
    ensure_socket(path, metadata.file_type())?;
    std::fs::remove_file(path).map_err(|error| platform_error("remove-stale-unix-helper", error))
}
        ...
        match std::fs::symlink_metadata(&self.path) {
            Ok(metadata) => {
                ...
                std::fs::remove_file(&self.path)
                    .map_err(|error| platform_error("remove-unix-helper-socket", error))
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
            Err(error) => Err(platform_error("inspect-unix-helper-socket", error)),
        }
    }
}
