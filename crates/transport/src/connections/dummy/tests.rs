...
#[test]
fn close_waits_for_claimed_dummy_dispatch_commit() {
...
#[tokio::test]
async fn queued_dummy_message_retains_raw_frame_capacity() {
...
#[tokio::test]
async fn receiver_capacity_drop_does_not_fail_irrevocable_dummy_send() {
    controlled::enable(true);
...
#[tokio::test]
async fn oversized_dummy_frame_is_dropped_before_receiver_dispatch() {
    controlled::enable(true);
    controlled::reset_sent_count();
    let capacity = Arc::new(InboundFrameCapacity::new());
...
#[tokio::test]
async fn pending_dummy_close_fences_an_irrevocable_dispatch_synchronously() {
    controlled::enable(true);
    controlled::reset_sent_count();
...
async fn cancelling_pending_close_completes_physical_retirement() {
    controlled::enable(true);
    controlled::set_close_pending(true);
...
#[test]
fn irrevocable_retirement_drop_outside_runtime_is_panic_free() {
...
#[tokio::test]
async fn failed_irrevocable_dummy_dispatch_retires_connection_and_rejects_later_send() {
    let local_callback = InnerTransportCallback::new_for_test(
        "local-peer",
        Box::new(NoopCallback),
        Notifier::default(),
