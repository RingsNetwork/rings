...
#[test]
fn test_connection_wide_fence_serializes_cross_channel_admission_with_retirement() {
...
#[tokio::test]
async fn test_cancelled_revocable_native_send_releases_task_owned_state() {
...
#[tokio::test]
async fn test_started_native_send_outlives_cancelled_caller() {
    let started = Arc::new(AtomicBool::new(false));
...
#[tokio::test]
async fn test_failed_irrevocable_send_retires_after_caller_is_cancelled() {
    let started = Arc::new(AtomicBool::new(false));
    let retired = Arc::new(AtomicBool::new(false));
    let release = Arc::new(tokio::sync::Notify::new());
    let permit = SendPermit::always();
...
#[tokio::test]
async fn test_accepted_send_is_not_retired_when_its_waiter_is_cancelled() {
...
#[tokio::test]
async fn test_cancellation_wins_before_native_background_send_becomes_irrevocable() {
...
#[test]
fn test_missing_runtime_returns_typed_error() {
    assert!(matches!(
        native_send_runtime(),
...
#[tokio::test]
async fn test_panicking_irrevocable_native_send_retires_connection() {
    let retired = Arc::new(AtomicBool::new(false));
    let connection_state = ConnectionStateCell::new();
    let cancellation = CancellationToken::new();
...
#[tokio::test]
async fn test_panicking_revocable_native_send_does_not_retire_connection() {
    let retired = Arc::new(AtomicBool::new(false));
    let connection_state = ConnectionStateCell::new();
    let cancellation = CancellationToken::new();
    let retirement_fence =
...
#[tokio::test]
async fn test_irrevocable_native_send_has_a_completion_bound() {
    let runtime = native_send_runtime().expect("Tokio test runtime must be available");
    let cancellation = CancellationToken::new();
...
#[tokio::test]
async fn test_failed_irrevocable_send_retires_before_preserving_its_error() {
    let retired = Arc::new(AtomicBool::new(false));
    let connection_state = ConnectionStateCell::new();
    let cancellation = CancellationToken::new();
    let permit = SendPermit::always();
