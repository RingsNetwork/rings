...
use crate::tests::wasm::within_hang_guard;
...
impl ObservedProcessor {
    ...
    async fn receive_texts(&self, count: usize) -> Vec<String> {
        ...
        let mut received = within_hang_guard(
            format_args!("{count} texts"),
            texts.by_ref().take(count).collect::<Vec<_>>(),
        )
        .await;
...
async fn create_connection(p1: &ObservedProcessor, p2: &ObservedProcessor) {
    ...
    futures::join!(
        p1.transitions.admitted(p2.processor.did()),
        p2.transitions.admitted(p1.processor.did()),
    );
}
...
///
/// ```text
/// Admitted(p1, p2) ∧ Admitted(p2, p1) ; sends  ⊢  ◇(inbox(p1) = {2, 3, 5}) ∧ ◇(inbox(p2) = {1, 4})
/// ```
///
...
/// route p1's offer to p3 and p3's answer back. Records are keyed by DID, so once both
/// admissions are observed, each end lists exactly one `Connected` entry for the other.
#[wasm_bindgen_test]
async fn test_processor_connect_with_did() {
    ...
    p3_connect.unwrap();
    futures::join!(
        p1.transitions.admitted(p3.processor.did()),
        p3.transitions.admitted(p1.processor.did()),
    );

    console_log!("check peers");
    assert_eq!(p1.peer_states(p3.processor.did()), ["Connected"]);
    assert_eq!(p3.peer_states(p1.processor.did()), ["Connected"]);
