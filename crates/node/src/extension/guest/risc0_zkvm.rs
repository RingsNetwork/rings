...
#[cfg(any(test, all(feature = "guest-riscv-prove", not(target_arch = "wasm32"))))]
use bytes::Bytes;
...
mod tests {
    ...
    fn hash(seed: u8) -> GuestProgramHash {
        match GuestProgramHash::new([seed; 32], "test") {
            Ok(hash) => hash,
    ...
    fn output() -> GuestStepOutput {
        GuestStepOutput {
            state: super::super::GuestState::new(Bytes::from_static(b"next")),
            effects: Vec::new(),
    ...
    fn journal_bytes_for_claim(public_input: GuestPublicInput, output_abi: Vec<u8>) -> Vec<u8> {
        let claim = Risc0JournalClaim {
            public_input,
            output_abi,
        };
        match ::risc0_zkvm::serde::to_vec(&claim) {
            Ok(words) => words
                .into_iter()
                .flat_map(u32::to_le_bytes)
                .collect::<Vec<_>>(),
            Err(error) => panic!("encode risc0 journal failed: {error}"),
        }
    }
    ...
    #[test]
    fn journal_decode_reads_public_input_and_guest_step_output_abi() {
        let output = output();
        let public_input = GuestPublicInput::new(Bytes::from_static(b"public-input"));
        let output_abi = match output.encode_abi() {
            Ok(bytes) => bytes,
            Err(error) => panic!("encode output ABI failed: {error}"),
        };
        let output_abi_len = output_abi.len();
        let journal = Journal::new(journal_bytes_for_claim(public_input.clone(), output_abi));
        let proven_journal = decode_proven_journal(&journal).expect("decode proven journal");
    ...
    #[test]
    fn proven_journal_match_requires_public_input_and_public_output() {
        let output = output();
