...
#[test]
fn per_peer_session_budget_rejects_only_the_saturated_peer() {
    let relay = web_relay();
...
#[test]
fn global_session_budget_rejects_without_allocating_state() {
    let relay = web_relay();
    let mut state = relay.init();
...
#[test]
fn backend_abort_removes_session_and_emits_one_peer_close() {
    let relay = web_relay();
    let opened = step_frame(&relay, &relay.init(), peer_a(), &open(7, "web"));
...
#[test]
fn tcp_shutdown_is_affine_and_blocks_late_peer_data_without_closing_reverse() {
    let relay = web_relay();
...
#[test]
fn udp_ignores_stream_shutdown_and_keeps_accepting_datagrams() {
    let mut config = HashMap::new();
...
#[test]
fn engine_model_refines_non_reusing_generation_exhaustion() {
...
#[test]
fn exhausted_session_allocator_rejects_accept_without_mutating_state() {
    let relay = web_relay();
    let mut state = relay.init();
...
#[test]
fn accepted_session_obeys_the_same_per_peer_budget() {
    let relay = web_relay();
    let mut state = relay.init();
    for session in 0..MAX_RELAY_SESSIONS_PER_PEER as u64 {
        assert!(state.insert_session(rkey(peer_a(), session)));
