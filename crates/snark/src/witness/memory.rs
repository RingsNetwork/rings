...
impl SafeMemory {
    ...
    pub fn free_pos(&self, store: &impl AsStoreRef) -> Result<u32> {
        self.read_u32(store, 0)
    }

    /// Sets the next free position in the memory
    pub fn set_free_pos(&mut self, store: &impl AsStoreRef, ptr: u32) -> Result<()> {
    ...
    pub fn write_u32(&mut self, store: &impl AsStoreRef, ptr: usize, num: u32) -> Result<()> {
        let view = self.view(store);
        let buf = unsafe { view.data_unchecked_mut() };
        let len = buf.len();
        let range = checked_range(len, ptr, std::mem::size_of::<u32>())?;
        buf.get_mut(range)
            .ok_or_else(|| memory_bounds_error(ptr, std::mem::size_of::<u32>(), len))?
            .copy_from_slice(&num.to_le_bytes());
        Ok(())
    }

    /// Reads a u32 from the specified memory offset
    pub fn read_u32(&self, store: &impl AsStoreRef, ptr: usize) -> Result<u32> {
        let view = self.view(store);
        let buf = unsafe { view.data_unchecked() };

        let mut bytes = [0; 4];
        let range = checked_range(buf.len(), ptr, std::mem::size_of::<u32>())?;
        bytes.copy_from_slice(
            buf.get(range)
                .ok_or_else(|| memory_bounds_error(ptr, std::mem::size_of::<u32>(), buf.len()))?,
        );
    ...
    pub fn read_fr<F: PrimeField>(&self, store: &impl AsStoreRef, ptr: usize) -> Result<F> {
        let view = self.view(store);
    ...
    fn write_short(&mut self, store: &impl AsStoreRef, ptr: usize, fr: U256) -> Result<()> {
        ...
        let mut low = [0u8; 4];
        for (dst, src) in low.iter_mut().zip(bytes.iter().take(4)) {
            *dst = *src;
        }
    ...
    fn write_big(&self, store: &impl AsStoreRef, ptr: usize, num: U256) -> Result<()> {
        ...
        let len = buf.len();
        let range = checked_range(len, ptr, 32)?;
        buf.get_mut(range)
            .ok_or_else(|| memory_bounds_error(ptr, 32, len))?
    ...
    pub fn read_big(&self, store: &impl AsStoreRef, ptr: usize) -> Result<U256> {
        let view = self.view(store);
        let buf = unsafe { view.data_unchecked() };

        let range = checked_range(buf.len(), ptr, 32)?;
        let bytes = buf
            .get(range)
            .ok_or_else(|| memory_bounds_error(ptr, 32, buf.len()))?;
        Ok(U256::from_le_slice(bytes))
    }
}

fn checked_range(len: usize, ptr: usize, width: usize) -> Result<std::ops::Range<usize>> {
    let end = ptr
        .checked_add(width)
        .ok_or_else(|| memory_bounds_error(ptr, width, len))?;
    if end > len {
...
fn memory_bounds_error(ptr: usize, width: usize, len: usize) -> Error {
    Error::WitnessWasmMemoryError(format!(
        "memory access out of bounds: ptr={ptr}, width={width}, len={len}"
    ))
}
