...
/// Load local r1cs
pub fn load_r1cs_local<F: PrimeField>(path: impl AsRef<std::path::Path>, format: Format) -> Result<R1CS<F>> {
    let ret = match format {
...
pub async fn load_r1cs<F: PrimeField>(path: Path, format: Format) -> Result<R1CS<F>> {
    let ret = match path {
	Path::Local(p) => {
	    load_r1cs_local::<F>(p, format)?
	},
	Path::Remote(url) => {
	    load_r1cs_remote::<F>(&url, format).await?
	}
    };
    Ok(ret)
}
...
/// Load witness local
pub fn load_witness_local<F: PrimeField>(path: impl AsRef<std::path::Path>, format: Format) -> Result<TyWitness<F>> {
    let ret = match format {
...
pub async fn load_witness<F: PrimeField>(path: Path, format: Format) -> Result<TyWitness<F>> {
    match path {
	Path::Local(p) => {
	    load_witness_local::<F>(p, format)
	},
	Path::Remote(url) => {
	    load_witness_remote::<F>(&url, format).await
	}
    }
}
...
pub fn load_circom_witness_calculator_local(path: impl AsRef<std::path::Path>) -> Result<WitnessCalculator> {
